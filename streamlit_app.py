...
def _category_files(key: str):
    s = f"backtest_results_summary_{key}.csv"
    d = f"backtest_results_detailed_{key}.csv"
    if not os.path.exists(s):
        s = "backtest_results_summary.csv"
    if not os.path.exists(d):
        d = "backtest_results_detailed.csv"
    return s, d
...
def load_benchmark_forward(holding_period_days: int = HOLDING_PERIOD):
    try:
        bm = pd.read_csv('nifty100_fileter_data.csv')
        bm['Date'] = pd.to_datetime(bm['Date'])
        bm = bm.sort_values('Date').set_index('Date')
        close = bm['Close'].astype(float)
        # Forward 6M return: (P[t+H]/P[t]) - 1
        fwd = close.shift(-holding_period_days) / close - 1
        return fwd
    except Exception:
        return None
...
def show_overview(summary, detailed):
    ...
    weights_perf = {
        ...
        "Momentum": {"ann_return":0.40, "sharpe":0.30, "beta":0.20, "mdd":0.10},
    ...
    dates_all = sorted(summary['rebalance_date'].dt.strftime('%Y-%m-%d').unique())
    bench_fwd = load_benchmark_forward()
    perf_rows = []
    for ds in dates_all:
        dd = detailed[detailed['rebalance_date'].dt.strftime('%Y-%m-%d') == ds].copy()
        if dd.empty:
        ...
        if 'forward_rank' not in dd.columns:
            dd['forward_rank'] = dd['forward_return'].rank(ascending=False, method='first')
        # Benchmark forward return for this date (nearest trading day)
        bench_ret = np.nan
        if bench_fwd is not None:
            ds_ts = pd.to_datetime(ds)
            try:
                idx = bench_fwd.index.get_indexer([ds_ts], method='nearest')[0]
                bench_ret = float(bench_fwd.iloc[idx])
            except Exception:
                bench_ret = np.nan
        for crit, wmap in weights_perf.items():
            need = list(wmap.keys())
    ...
    if not perf_df.empty:
        def _agg(g):
            ...
            return pd.Series({
                'Avg 6M Return (%)': mfr.mean()*100 if len(mfr)>0 else np.nan,
                'Benchmark Avg 6M Return (%)': bfr.mean()*100 if len(bfr)>0 else np.nan,
...
def show_fund_rankings(summary, detailed):
    ...
    if show_all:
        ...
        all_needed = ['sharpe','sortino','mdd','ann_return','ann_vol','beta','te','ir']
        for m in all_needed:
            if m not in d.columns:
                d[m] = np.nan
    ...
    if sel_rows:
        ...
        avg_ret = sel_all.groupby('scheme_code')['forward_return'].mean().rename('Avg 6M Return')
