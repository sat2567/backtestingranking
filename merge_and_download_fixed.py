"""
Fixed version of merge and download script (no unicode characters)
"""
...
def merge_largecap_files():
    ...
    print("SUCCESS: Merged largecap file saved!")
...
def download_nifty100_data():
    ...
    try:
        ...
        if nifty.empty:
            print(f"WARNING: No data for {ticker}, trying alternative ticker ^NSEI...")
            ticker = "^NSEI"
            nifty = yf.download(ticker, start="2006-01-01", end=datetime.now().strftime("%Y-%m-%d"), progress=False)
        ...
        print(f"SUCCESS: Downloaded {len(nifty)} days of data")
        ...
        print(f"SUCCESS: Nifty 100 data saved to: {output_path}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("=" * 60)
        if merged_file:
            print("SUCCESS: Merged largecap file: largecap_merged.xlsx")
        if nifty_file:
            print("SUCCESS: Nifty 100 data: nifty100_data.csv")
