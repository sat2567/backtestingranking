...
# 260 trading days/year, fixed exponent
# 1Y: window=260, exp=260/260=1  → simple return
# 3Y: window=780, exp=260/780=1/3
# 5Y: window=1300, exp=260/1300=1/5
# ============================================================================
...
def main():
    ...
    # Add Nifty row at top for reference
    nifty_row = {'Fund Name': '📊 Nifty 100 (Benchmark)', 'Since': '—', 'Years': '—'}
    for label in ['1Y', '3Y', '5Y']:
        nifty_row[f'{label} Mean %']   = nifty_stats.get(label, {}).get('Mean',   np.nan)
        nifty_row[f'{label} Median %'] = nifty_stats.get(label, {}).get('Median', np.nan)
        nifty_row[f'{label} Min %']    = nifty_stats.get(label, {}).get('Min',    np.nan)
    ...
    if selected_name != '— Show Summary Table Only —':
        ...
        # Align nifty to fund start date
        nifty_aligned = None
        if nifty is not None:
            nifty_aligned = nifty[nifty.index >= fund_series.index.min()]
        ...
        # Metric cards
        render_fund_cards(fund_row, nifty_stats)
        ...
        for chart_title, window, plabel in periods:
            ...
            fig = comparison_chart(fund_series, nifty_aligned, selected_name, window, plabel)
            if fig:
                # Annotate with fund stats
                f_stats = rolling_returns(fund_series, window)
                ann_text = (
                    f"Fund — Mean: {f_stats.mean()*100:.2f}%  |  "
                    f"Median: {f_stats.median()*100:.2f}%  |  "
                    f"Min: {f_stats.min()*100:.2f}%"
                )
                if nifty_aligned is not None:
                    n_stats = rolling_returns(nifty_aligned, window)
                    if len(n_stats) > 0:
                        ann_text += (
                            f"     |     Nifty — Mean: {n_stats.mean()*100:.2f}%  |  "
                            f"Median: {n_stats.median()*100:.2f}%  |  "
                            f"Min: {n_stats.min()*100:.2f}%"
                        )
                st.caption(ann_text)
